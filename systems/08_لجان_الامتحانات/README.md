# 📁 نظام جدولة الاختبارات وتوزيع اللجان (ETDS)
## Exam Timetabling & Distribution System — v2.0
...
| **عدد الجداول** | 20 جدول + 6 Views + 1 SP |
| **الملفات** | `DDL.sql` + `DDL_SCHEDULING.sql` |
| **الإصدار** | 2.0 |

---

## 🚀 المقدمة
نظام ETDS يجمع بين **جدولة الاختبارات** (متى يُختبَر كل صف/مادة؟) و**توزيع اللجان** (أين يجلس كل طالب؟). يضمن العدالة، ويمنع الغش، ويوفر الجهد الهائل في الترتيب اليدوي.

### 🆕 الجديد في v2.0 (نظام الجدولة `DDL_SCHEDULING.sql`)
- **جدول موحد لأنواع الاختبارات** (`lookup_exam_types`): شهري، نصفي، نهائي، عملي، أخرى
- **فترات زمنية موحدة** (`lookup_exam_time_slots`): 8-10، 10-12، إلخ
- **جدول الاختبارات** (`exam_timetable`): يربط كل اختبار بالمادة والصف والتاريخ والفترة ونوع المكان
- **مراقبون** (`exam_proctors`): ربط المعلمين بالجلسات مع أدوار
- **منع التعارض** (Triggers): الجدولة لنفس الصف/المادة/التاريخ + التقويم المدرسي
...
**تم التحديث:** 2026-02-19 (v2.0 — إضافة DDL_SCHEDULING.sql)
